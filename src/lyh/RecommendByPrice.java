...
public class RecommendByPrice implements Tool {
    //按起飞地点找
    public ObservableList<Flight> SeekTakeoff(String place1){
        ...
        try {
            ...
            ResultSet rs = stmt.executeQuery("select * from flight.flight where place1 = '" + place1 + "'");
            while (rs.next()) {
    ...
    //按降落地点找
    public ObservableList<Flight> SeekLanding(String place3){
        ObservableList<Flight> flightList = FXCollections.observableArrayList();
        Connection conn = null;
        FlightOperation operation = new FlightOperation();
        try {
            conn = DatabaseConnection.getCon();
            Statement stmt = conn.createStatement();
            ResultSet rs = stmt.executeQuery("select * from flight.flight where place2 = '" + place3 + "'");
            while (rs.next()) {
                Flight flight = new Flight();
                flight.setFlightId(rs.getString("flight_id"));
                flight.setAirway(rs.getString("airway"));
    ...
    public ObservableList<Flight2> FlightMatches2(ObservableList<Flight> takeoff,ObservableList<Flight> landing){
        ...
        for(int i=0;i<takeoff.size();i++)
            for(int j=0;i<landing.size();j++){
                ...
                String laning=takeoff.get(j).getPlace1();
                if(takoff.equals(laning)){
                    flightList.add(takeoff.get(i));
                    flightList.add(landing.get(j));
                }
    ...
    public ObservableList<Flight2> FlightMatches3(ObservableList<Flight> takeoff,ObservableList<Flight> landing){
        ...
        for(int i=0;i<takeoff.size();i++)
            for(int j=0;i<landing.size();j++){
                if(takeoff.get(i).getIsStop()&&landing.get(i).getIsStop()){
                    String takoff = takeoff.get(i).getPlace2();
                    String laning = takeoff.get(j).getPlace2();
                    if (takoff.equals(laning)) {
                        flightList.add(takeoff.get(i));
                        flightList.add(landing.get(j));
                    }
    ...
    public ObservableList<Flight2> FlightMatches4(ObservableList<Flight> takeoff,ObservableList<Flight> landing){
        ...
        for(int i=0;i<takeoff.size();i++)
            for(int j=0;i<landing.size();j++){
                ...
                String laning=takeoff.get(j).getPlace3();
                if(takoff.equals(laning)){
                    flightList.add(takeoff.get(i));
                    flightList.add(landing.get(j));
                }
    ...
    public ObservableList<Flight2> Flightunion(ObservableList<Flight2> fli1,ObservableList<Flight2> fli2,ObservableList<Flight2> fli3,ObservableList<Flight2> fli4){
        ...
        for(int j=0;j<fli1.size();j++){
            flightList.add(fli2.get(j));
        }
        for(int k=0;k<fli1.size();k++){
            flightList.add(fli3.get(k));
        }
        for(int m=0;m<fli1.size();m++){
    ...
    public ObservableList<Flight2> FlightPriceRecommend(ObservableList<Flight2> fliList){
        ...
        try {
            for (int i = 0; i < fliList.size(); i = i + 2) {
                ...
                if (minprice == 0) {
                    minprice = price;
                    flightList.add(0, flightList.get(i));
                    flightList.add(1, flightList.get(i + 1));
                } else if (price < minprice&&flightList.get(i).getTicket()!=0&&flightList.get(i+1).getTicket()!=0) {
                    minprice = price;
    ...
    public ObservableList<Flight2> PriceRecommend(String place1,String place2){
        ObservableList<Flight> SeekTakeoff = FXCollections.observableArrayList();
        ObservableList<Flight> SeekLanding = FXCollections.observableArrayList();
        ObservableList<Flight2> FlightMatches1 = FXCollections.observableArrayList();
        ObservableList<Flight2> FlightMatches2 = FXCollections.observableArrayList();
        ObservableList<Flight2> FlightMatches3 = FXCollections.observableArrayList();
        ObservableList<Flight2> FlightMatches4 = FXCollections.observableArrayList();
        ObservableList<Flight2> Flightunion = FXCollections.observableArrayList();
        ObservableList<Flight2> flightPriceRecommend = FXCollections.observableArrayList();
        RecommendByPrice op=new RecommendByPrice();
        SeekTakeoff=op.SeekTakeoff(place1);
        SeekLanding=op.SeekLanding(place2);
        FlightMatches1=op.FlightMatches1(SeekTakeoff,SeekLanding);
        FlightMatches2=op.FlightMatches2(SeekTakeoff,SeekLanding);
        FlightMatches3=op.FlightMatches3(SeekTakeoff,SeekLanding);
        FlightMatches4=op.FlightMatches4(SeekTakeoff,SeekLanding);
        Flightunion=op.Flightunion(FlightMatches1,FlightMatches2,FlightMatches3,FlightMatches4);
        flightPriceRecommend=op.FlightPriceRecommend(Flightunion);
